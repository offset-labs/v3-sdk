import { Token, CurrencyAmount, WETH9 } from '@offsetcarbon/sdk-core'
...
describe('Pool', () => {
  const USDC = new Token(421614, '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', 6, 'USDC', 'USD Coin')
  const DAI = new Token(421614, '0x6B175474E89094C44Da98b954EedeAC495271d0F', 18, 'DAI', 'DAI Stablecoin')

  describe('constructor', () => {
    // only one chain currently supported
    // it('cannot be used for tokens on different chains', () => {
    //   expect(() => {
    //     new Pool(USDC, WETH9[421614], FeeAmount.MEDIUM, encodeSqrtRatioX96(1, 1), 0, 0, [])
    //   }).toThrow('CHAIN_IDS')
    // })

    it('fee must be integer', () => {
      expect(() => {
        new Pool(USDC, WETH9[421614], FeeAmount.MEDIUM + 0.5, encodeSqrtRatioX96(1, 1), 0, 0, [])
      }).toThrow('FEE')
    })

    it('fee cannot be more than 1e6', () => {
      expect(() => {
        new Pool(USDC, WETH9[421614], 1e6, encodeSqrtRatioX96(1, 1), 0, 0, [])
      }).toThrow('FEE')
    })
    ...
    it('price must be within tick price bounds', () => {
      expect(() => {
        new Pool(USDC, WETH9[421614], FeeAmount.MEDIUM, encodeSqrtRatioX96(1, 1), 0, 1, [])
      }).toThrow('PRICE_BOUNDS')
      expect(() => {
        new Pool(USDC, WETH9[421614], FeeAmount.MEDIUM, JSBI.add(encodeSqrtRatioX96(1, 1), JSBI.BigInt(1)), 0, -1, [])
      }).toThrow('PRICE_BOUNDS')
    })
    ...
    it('works with valid arguments for empty pool low fee', () => {
      new Pool(USDC, WETH9[421614], FeeAmount.LOW, encodeSqrtRatioX96(1, 1), 0, 0, [])
    })

    it('works with valid arguments for empty pool lowest fee', () => {
      new Pool(USDC, WETH9[421614], FeeAmount.LOWEST, encodeSqrtRatioX96(1, 1), 0, 0, [])
    })

    it('works with valid arguments for empty pool high fee', () => {
      new Pool(USDC, WETH9[421614], FeeAmount.HIGH, encodeSqrtRatioX96(1, 1), 0, 0, [])
    })
  })
  ...
  describe('#priceOf', () => {
    ...
    it('throws if invalid token', () => {
      expect(() => pool.priceOf(WETH9[421614])).toThrow('TOKEN')
    })
  })
  ...
  describe('#chainId', () => {
    it('returns the token0 chainId', () => {
      let pool = new Pool(USDC, DAI, FeeAmount.LOW, encodeSqrtRatioX96(1, 1), 0, 0, [])
      expect(pool.chainId).toEqual(421614)
      pool = new Pool(DAI, USDC, FeeAmount.LOW, encodeSqrtRatioX96(1, 1), 0, 0, [])
      expect(pool.chainId).toEqual(421614)
    })
  })
  ...
  describe('#involvesToken', () => {
    ...
    expect(pool.involvesToken(WETH9[421614])).toEqual(false)
  })
